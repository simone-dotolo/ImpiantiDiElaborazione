...
df = pd.read_excel('HomeWork_Regression.xls', sheet_name='os2')

slope_vmsize, intercept_vmsize, r_value_vmsize, p_value_vmsize, std_err_vmsize = st.linregress(df['TIME'], df['LIN2_VmSize'])
slope_vmdata, intercept_vmdata, r_value_vmdata, p_value_vmdata, std_err_vmdata = st.linregress(df['TIME'], df['LIN2_VmData'])
slope_rss, intercept_rss, r_value_rss, p_value_rss, std_err_rss = st.linregress(df['TIME'], df['LIN2_RSS'])
slope_byteletti_sec, intercept_byteletti_sec, r_value_byteletti_sec, p_value_byteletti_sec, std_err_byteletti_sec = st.linregress(df['TIME'], df['LIN2_byte_letti__sec'])
slope_bytescritti_sec, intercept_bytescritti_sec, r_value_bytescritti_sec, p_value_bytescritti_sec, std_err_bytescritti_sec = st.linregress(df['TIME'], df['LIN2_byte_scritti__sec'])

plt.figure(figsize=(15,8))

plt.subplot(231)
plt.title(f'VmSize | R2: {r_value_vmsize**2}')
plt.scatter(df['TIME'], df['LIN2_VmSize'], color='b')
plt.plot(df['TIME'], df['TIME']*slope_vmsize + intercept_vmsize, color='r')

plt.subplot(232)
plt.title(f'VmData | R2: {r_value_vmdata**2}')
plt.scatter(df['TIME'], df['LIN2_VmData'], color='b')
plt.plot(df['TIME'], df['TIME']*slope_vmdata + intercept_vmdata, color='r')

plt.subplot(233)
plt.title(f'RSS | R2: {r_value_rss**2}')
plt.scatter(df['TIME'], df['LIN2_RSS'], color='b')
plt.plot(df['TIME'], df['TIME']*slope_rss + intercept_rss, color='r')

plt.subplot(234)
plt.title(f'ByteLetti_sec | R2: {r_value_byteletti_sec**2}')
plt.scatter(df['TIME'], df['LIN2_byte_letti__sec'], color='b')
...
plt.scatter(df['TIME'], df['LIN2_byte_scritti__sec'], color='b')
plt.plot(df['TIME'], df['TIME']*slope_bytescritti_sec + intercept_bytescritti_sec, color='r')

plt.show()

residui_vmsize = df['LIN2_VmSize'] - (df['TIME']*slope_vmsize + intercept_vmsize)
residui_vmdata = df['LIN2_VmData'] - (df['TIME']*slope_vmdata + intercept_vmdata)
residui_rss = df['LIN2_RSS'] - (df['TIME']*slope_rss + intercept_rss)
residui_byteletti_sec = df['LIN2_byte_letti__sec'] - (df['TIME']*slope_byteletti_sec + intercept_byteletti_sec)
residui_bytescritti_sec = df['LIN2_byte_scritti__sec'] - (df['TIME']*slope_bytescritti_sec + intercept_bytescritti_sec)
...
_, kendall_pvalue_vmsize = st.kendalltau(df['TIME'], df['LIN2_VmSize'])
_, kendall_pvalue_vmdata = st.kendalltau(df['TIME'], df['LIN2_VmData'])
_, kendall_pvalue_rss = st.kendalltau(df['TIME'], df['LIN2_RSS'])
_, kendall_pvalue_byteletti_sec = st.kendalltau(df['TIME'], df['LIN2_byte_letti__sec'])
_, kendall_pvalue_bytescritti_sec = st.kendalltau(df['TIME'], df['LIN2_byte_scritti__sec'])
...
theils_slope_vmsize, theils_intercept_vmsize, theils_low_slope_vmsize, theils_high_slope_vmsize = st.theilslopes(df['LIN2_VmSize'], df['TIME'])
...
theils_slope_vmdata, theils_intercept_vmdata, theils_low_slope_vmdata, theils_high_slope_vmdata = st.theilslopes(df['LIN2_VmData'], df['TIME'])
...
theils_slope_rss, theils_intercept_rss, theils_low_slope_rss, theils_high_slope_rss = st.theilslopes(df['LIN2_RSS'], df['TIME'])
...
theils_slope_byteletti_sec, theils_intercept_byteletti_sec, theils_low_slope_byteletti_sec, theils_high_slope_byteletti_sec = st.theilslopes(df['LIN2_byte_letti__sec'], df['TIME'])
...
theils_slope_bytescritti_sec, theils_intercept_bytescritti_sec, theils_low_slope_bytescritti_sec, theils_high_slope_bytescritti_sec = st.theilslopes(df['LIN2_byte_scritti__sec'], df['TIME'])

print('Regressione Lineare Robusta (Theil e Sen) per ByteScritti_sec')
print(f'Slope: {theils_slope_bytescritti_sec}')
print(f'Intercept: {theils_intercept_bytescritti_sec}')
print(f'Low Slope: {theils_low_slope_bytescritti_sec}')
print(f'High Slope: {theils_high_slope_bytescritti_sec}')
...
plt.subplot(236)
plt.title(f'ByteScritti_sec and robust linear regression')
plt.scatter(df['TIME'], df['LIN2_byte_scritti__sec'], color='b')
plt.plot(df['TIME'], df['TIME']*theils_slope_bytescritti_sec + theils_intercept_bytescritti_sec, color='r')
plt.plot(df['TIME'], df['TIME']*slope_bytescritti_sec + intercept_bytescritti_sec, color='g')
