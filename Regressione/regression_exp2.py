...
df = pd.read_excel('HomeWork_Regression.xls', sheet_name='EXP2')
...
theils_slope_nmail, theils_intercept_nmail, theils_low_slope_nmail, theils_high_slope_nmail = st.theilslopes(df['nmail'], df['observation'])

print('Regressione Lineare Robusta (Theil e Sen) per nmail')
print(f'Slope: {theils_slope_nmail}')
print(f'Intercept: {theils_intercept_nmail}')
print(f'Low Slope: {theils_low_slope_nmail}')
print(f'High Slope: {theils_high_slope_nmail}')

theils_slope_byte_rec, theils_intercept_byte_rec, theils_low_slope_byte_rec, theils_high_slope_byte_rec = st.theilslopes(df['byte rec'], df['observation'])

print('Regressione Lineare Robusta (Theil e Sen) per byte rec')
print(f'Slope: {theils_slope_byte_rec}')
print(f'Intercept: {theils_intercept_byte_rec}')
print(f'Low Slope: {theils_low_slope_byte_rec}')
print(f'High Slope: {theils_high_slope_byte_rec}')
...
plt.subplot(131)
plt.title(f'nmail and robust linear regression')
plt.scatter(df['observation'], df['nmail'], color='b')
plt.plot(df['observation'], df['observation']*theils_slope_nmail + theils_intercept_nmail, color='r')
plt.plot(df['observation'], df['observation']*slope_nmail + intercept_nmail, color='g')

plt.subplot(132)
plt.title(f'byte rec and robust linear regression')
plt.scatter(df['observation'], df['byte rec'], color='b')
plt.plot(df['observation'], df['observation']*theils_slope_byte_rec + theils_intercept_byte_rec, color='r')
plt.plot(df['observation'], df['observation']*slope_byte_rec + intercept_byte_rec, color='g')
