...
df = f.select_dtypes(['number']).dropna(axis=1)

# Dropping constant columns
print(*(df.columns[df.std() == 0]))
...
f['Cluster'] = ward.labels_

labels = sorted(f['label'].unique())

clusters = sorted(f['Cluster'].unique())

synthetic_workload_table = pd.DataFrame(columns=labels, index=clusters)

for cluster in clusters:
	for label in labels:
		count_cluster_label = len(f[(f['Cluster'] == cluster) & (f['label'] == label)])
		synthetic_workload_table.at[cluster, label] = count_cluster_label

elements_for_cluster = [sum(synthetic_workload_table.iloc[cluster]) for cluster in clusters]

for cluster in clusters:
	for label in labels:
		synthetic_workload_table.at[cluster, label] = synthetic_workload_table.at[cluster, label] / elements_for_cluster[cluster]

synthetic_workload_table.to_csv('SyntheticWorkloadTable.csv')
