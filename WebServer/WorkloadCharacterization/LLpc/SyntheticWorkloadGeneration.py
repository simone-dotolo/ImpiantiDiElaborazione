...
f = pd.read_table('WC_Synthetic.txt', skiprows=1, sep=r"\s+")
...
synthetic_workload = synthetic_workload.select_dtypes(['number']).dropna(axis=1)
synthetic_workload = synthetic_workload.drop(columns=synthetic_workload.std()[synthetic_workload.std() == 0].index)
synthetic_workload.to_csv('LLpc.csv')
